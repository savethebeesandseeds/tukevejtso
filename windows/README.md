...
`tk transcription` starts Enchanted Transcription: local Whisper capture for microphone and system output with an optional OpenAI-powered Agent Insights pane. F9 provides persistent transcription, agent, reference-context, transparency, and API-safety settings. Local capture and context buffering continue when API requests are paused. See the [Enchanted Transcription guide](agents/enchanted-transcription/README.md) for controls, defaults, privacy behavior, and private `.md`, `.txt`, `.json`, or `.csv` reference documents.
