...
Enhanced Typing and Enchanted Transcription are separate thin binaries over `..\speech-agent-core`, so shared audio capture, Whisper, transcript reconciliation, and Windows lifecycle fixes are implemented once without mixing the two product entrypoints.
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File C:\Work\tukevejtso\windows\agents\enhanced-typing\run.ps1 -AgentModel gpt-5.6-terra
```
